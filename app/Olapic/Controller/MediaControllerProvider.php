...
class MediaControllerProvider implements ControllerProviderInterface {
  ...
  function getLocation(Application $app, Request $request, $media_id, $network_id = false, $access_token = false) {
    $network = ($network_id) ? $network_id : trim($request->query->get('network'));
    $access_token = ($access_token)? $access_token : trim($request->query->get('access_token'));      
