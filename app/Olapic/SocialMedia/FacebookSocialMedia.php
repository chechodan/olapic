...
class FacebookSocialMedia implements SocialMediaInterface 
{
  ...
  static public function getLocationInfo($lat, $lng, $access_token, $name='', $limit=1, $distance=100) 
  {  
    $name = "q=".$name;
    $type = "type=place";
    $limit = "limit=".$limit;
    ...
    try
    {
      $location = json_decode(file_get_contents("https://graph.facebook.com/search?".$name.$lt.$type.$lt.$center.$lt.$limit.$lt.$distance.$lt.$access_token));
    } catch (\Exception $ex) { }
    ...
    if ($location && isset($location->data[0]->location))
    {
      ...
      $result = array("street" => $location->street, "city" => $location->city, "state" => $location->state, "country" => $location->country, "zip" => $location->zip, "latitude" => $location->latitude, "longitude" => $location->longitude, "geopoint" => $geopoint);
    }
