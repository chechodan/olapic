...
class FacebookMediaTest extends MediaTest 
{
  ...
  public function testEmptyAccessToken() {
    ...
    $this->assertJsonMessage($response, FacebookSocialMedia::MESSAGE_NOT_IMPLEMENTED);
  }
   
  public function testIncorrectAccessToken() {
    $response = $this->getResponse($this->media_id, $this->incorrect_access_token); 
  ...
  public function testIncorrectMediaID() {  
    $response = $this->getResponse($this->media_id, $this->facebook_access_token);  
  ...
  protected function getResponse($media_id, $access_token){
    ...
    $client->request('GET', '/media/'.$media_id.'?network=facebook&access_token='.$access_token);
